...
注：该部分文档存在大量非即时信息，可能存在部分错误，且可能在之后重写。
...
-   **方法池管理**：动态导入和装饰器系统已稳定。
-   **多模态和向量化支持**：由于该项目属于 **内存/数据搬运密集型** 因此不建议开启向量化，如需向量化，建议修改gp中的逻辑，或直接内嵌在方法中
...
## 未来工作
-   **添加模仿能力**：目前缺失的主要部分，预计添加后实用性应该能上升一个台阶。
-   **加强知识提取**：预备为知识提取部分添加贝叶斯选项，以及增加当前基于频率的知识提取的超参数
-   **升级LayerGrowTrainer**：
...
-   **可视化**
-   与神经网络类算法的双向协同优化（大概率不会做或者可能很后面，有感兴趣的可以试试）
-   预备添加基于哈希的优化，避免进化时出现大量相同图以及冗余图
