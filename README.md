...
### 示例
假设最小完备函数集 $F_{\text{完备}} = \{f_1, f_2\}$，比较两种功能集：
1. **精简功能集** Q₁ = {f₁, f₂, f₃}：

$$
C(Q) = \frac{2}{3}, \quad E(Q) \propto \frac{1}{(2/3)^\alpha + \eta}
$$
...
$$
C(Q) = \frac{2}{5}, \quad E(Q) \propto \frac{1}{(2/5)^\alpha + \eta}
$$
...
#### 效率对比结论
- 当 `α` 较大时：低完备性集（Q₁）效率显著更高
- 当 `η` 较大时：高完备性集（Q₂）可能更优
...
解空间维度不仅与功能集大小相关，还受以下因素影响：

$$
\text{dim}(\mathcal{S}_Q) \propto R^{|Q| \cdot d}
$$

- $R$：单个函数的输入空间维度  
- $|Q|$：功能集规模  
- $d$：网络深度或函数组合的调用链深度  
- 输入输出比（I/O比）过高或过低都可能导致信息瓶颈，影响解空间的实际可达区域。

> 因此，除了函数数量本身，**函数的组合深度与输入输出通道的匹配性，也决定了解空间的“有效容量”。**
