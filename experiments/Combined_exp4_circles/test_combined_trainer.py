# -*- coding: utf-8 -*-
# File: experiments/Combined_exp4_circles/experiment_4_combined_trainer_v4.py
import logging
import numpy as np
import time
import copy
...
# --- 3. 初始化 AdaptoFlux 实例 ---
base_adaptoflux = AdaptoFlux(values=input_data, labels=target, methods_path="experiments/Combined_exp4_circles/methods.py")
...
combined_config_base = {
    ...
    "verbose": False,
    "save_dir": "experiments/Combined_exp4_circles/temp_test_runs"
}
...
def run_group_a(base_af, input_data, target, test_input, test_target, lg_config, lg_train_kwargs, run_seed):
    ...
    save_path = f"experiments/Combined_exp4_circles/models/group_a_run_{run_seed}"
    ...
    # <<< MODIFIED >>>: 保存双集决策边界图
    plot_dir = "experiments/Combined_exp4_circles/plots"
...
def run_group_b(base_af, input_data, target, test_input, test_target, ge_config, ge_train_kwargs, run_seed):
    ...
    save_path = f"experiments/Combined_exp4_circles/models/group_b_run_{run_seed}"
    if os.path.exists(save_path): shutil.rmtree(save_path)
    os.makedirs(save_path, exist_ok=True)
    start_time = time.time()
...
def run_group_c(base_af, input_data, target, test_input, test_target, lg_config, ge_config, lg_train_kwargs, ge_train_kwargs, run_seed):
    ...
    lg_save_path = f"experiments/Combined_exp4_circles/models/group_c_lg_run_{run_seed}"
    ...
    ge_save_path = f"experiments/Combined_exp4_circles/models/group_c_ge_run_{run_seed}"
...
def run_group_d(base_af, input_data, target, test_input, test_target, config, genetic_mode, lg_train_kwargs, ge_train_kwargs, run_seed):
    ...
    save_path = f"experiments/Combined_exp4_circles/models/group_d_{genetic_mode}_run_{run_seed}"
    if os.path.exists(save_path): shutil.rmtree(save_path)
    os.makedirs(save_path, exist_ok=True)
...
# 保存结果
os.makedirs("experiments/Combined_exp4_circles/experiment_results", exist_ok=True)
with open("experiments/Combined_exp4_circles/experiment_results/exp4_combined_trainer_all_repeats_v4.json", "w", encoding='utf-8') as f:
    json.dump(all_results, f, indent=4, default=str, ensure_ascii=False)
with open("experiments/Combined_exp4_circles/experiment_results/exp4_combined_trainer_aggregated_v4.json", "w", encoding='utf-8') as f:
...
plt.tight_layout()
plt.savefig("experiments/Combined_exp4_circles/experiment_results/exp4_combined_trainer_comparison_v4.png", dpi=300, bbox_inches='tight')
plt.show()

# 生成报告（含 Train/Test）<<< MODIFIED >>> #
report_path = "experiments/Combined_exp4_circles/experiment_results/exp4_combined_trainer_report_v4.txt"
