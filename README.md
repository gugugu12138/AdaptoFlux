...
AdaptoFlux 是一种基于“功能集 + 路径搜索”的智能算法框架，核心思路是通过组合基础函数构造动态计算路径，以图结构（DFG）进行组织，并支持路径可解释性与结构优化。

主要特点：
- 使用函数集（包括 MLP、随机森林等）作为推理构件；
...
### 应用示例
集成运算中，将各个基分类器的输出作为模型的输入，使用简单的位运算和带权运算作为功能集
可结合多个分类器