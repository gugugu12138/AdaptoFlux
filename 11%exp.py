...
if __name__ == "__main__":
    ...
    model.load_paths_from_file("11%.txt")
    # print('使用sum作为坍缩方法')
    # model.evaluate(x_train.reshape(x_train.shape[0], -1), y_train)
    print('使用_collapse_sum_round作为坍缩方法')
    model.add_collapse_method(_collapse_sum_ceil)
    model.evaluate(x_train.reshape(x_train.shape[0], -1), y_train, save_errors_to="errors.json")
